...
import { fetchAnimeList, genres } from "@/lib/anime-data";
...
function BrowseContent() {
  ...
  const filteredAnime = useMemo(() => {
    ...
    if (selectedGenre) results = results.filter(a => a.genres.includes(selectedGenre));
    ...
    return results;
  }, [animeList, searchQuery, selectedGenre, sortBy, statusFilter, typeFilter]);

  const hasActiveFilters = selectedGenre || statusFilter !== "all" || typeFilter !== "all";

  return (
    <div className="min-h-screen bg-bg-main">
      <div className="mx-auto max-w-[1400px] px-4 py-8 lg:px-8">
        <div className="mb-8">
          <h1 className="font-heading text-3xl font-bold text-text-main md:text-4xl">Browse Anime</h1>
          <p className="mt-1 text-sm text-text-muted">Discover your next favorite series from our collection</p>
        </div>
        ...
        {showFilters && (
          <div className="mb-6 rounded-xl border border-border-main bg-bg-panel p-5">
            <div className="flex flex-col gap-5">
              <div>
                <h3 className="mb-2 text-sm font-semibold text-text-main">Genres</h3>
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => setSelectedGenre("")}
                    className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${!selectedGenre ? "bg-green-main text-bg-main" : "bg-bg-card text-text-secondary hover:text-text-main"}`}>
                    All
                  </button>
                  {genres.map(genre => (
                    <button key={genre} type="button" onClick={() => setSelectedGenre(selectedGenre === genre ? "" : genre)}
                      className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${selectedGenre === genre ? "bg-green-main text-bg-main" : "bg-bg-card text-text-secondary hover:text-text-main"}`}>
                      {genre}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex flex-col gap-5 sm:flex-row sm:gap-8">
                <div>
                  <h3 className="mb-2 text-sm font-semibold text-text-main">Status</h3>
                  <div className="flex flex-wrap gap-2">
                    {(["all", "Airing", "Completed", "Upcoming"] as const).map(status => (
                      <button key={status} type="button" onClick={() => setStatusFilter(status)}
                        className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${statusFilter === status ? "bg-green-main text-bg-main" : "bg-bg-card text-text-secondary hover:text-text-main"}`}>
                ...
                <div>
                  <h3 className="mb-2 text-sm font-semibold text-text-main">Type</h3>
                  <div className="flex flex-wrap gap-2">
                    {(["all", "TV", "Movie", "OVA", "ONA", "Special"] as const).map(type => (
                      <button key={type} type="button" onClick={() => setTypeFilter(type)}
                        className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${typeFilter === type ? "bg-green-main text-bg-main" : "bg-bg-card text-text-secondary hover:text-text-main"}`}>
                        {type === "all" ? "All" : type}
                      </button>
                    ))}
                  </div>
              ...
              {hasActiveFilters && (
                <button type="button" onClick={() => { setSelectedGenre(""); setStatusFilter("all"); setTypeFilter("all"); }}
        ...
        <div className="mb-4 text-sm text-text-muted">
          {loading ? "Loading..." : `${filteredAnime.length} anime found`}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <h3 className="mb-1 text-lg font-semibold text-text-main">No results found</h3>
